function createUI() {
  ...
  var beatDelayGroup = window.add("group");
  beatDelayGroup.add(
    "statictext",
    undefined,
    "Start numbering from this beat: "
  );
  var beatDelayInput = beatDelayGroup.add("edittext", undefined, "1");
  beatDelayInput.characters = 1;

  var intervalGroup = window.add("group");
  var checkboxFR = intervalGroup.add(
    "checkbox",
    undefined,
    "Stick to framerate?"
  );
  ...
  addButton.onClick = function () {
    ...
    var beatDelay = parseFloat(beatDelayInput.text) - 1;
    ...
    for (i = 0; i < count; i++) {
      ...
      layer
        ...
        .setValueAtTime(markerTime, new MarkerValue(((i + beatDelay) % 4) + 1));
    }
    layer.enabled = false;
    layer.locked = true;
    window.close();
